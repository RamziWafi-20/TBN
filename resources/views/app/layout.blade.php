...
<head>
    ...
    <title>@yield('title', 'TBN Dashboard')</title>
...
<header class="topbar">
    <div class="topbar-inner">
        <a href="{{ route('beranda') }}" class="brand"><span class="brand-icon">🌿</span><span>TBN</span></a>
        <nav class="nav">
            <a href="{{ route('beranda') }}" class="{{ request()->routeIs('beranda','dashboard') ? 'active' : '' }}">Beranda</a>
            <a href="{{ route('ranking') }}" class="{{ request()->routeIs('ranking') ? 'active' : '' }}">🏆 Ranking</a>
            <a href="{{ route('income') }}" class="{{ request()->routeIs('income') ? 'active' : '' }}">💰 Penghasilan</a>
            <a href="{{ route('profile') }}" class="{{ request()->routeIs('profile') ? 'active' : '' }}">👤 Profil</a>
            <a href="{{ route('points') }}" class="{{ request()->routeIs('points*') ? 'active' : '' }}">⭐ Poin</a>
            @if(in_array($user->role, ['Pengelola', 'Guru'], true))<a href="{{ route('vouchers') }}" class="{{ request()->routeIs('vouchers*') ? 'active' : '' }}">🎟️ Voucher</a>@endif
        </nav>
        <button type="button" class="theme-toggle" id="themeToggle" title="Mode gelap">🌙</button>
        <div class="user-mini">
            @if($user->profile_photo)
                <img class="avatar" src="{{ asset('storage/'.$user->profile_photo) }}" alt="Foto profil">
            @else
                <div class="avatar">{{ strtoupper(substr($user->name,0,1)) }}</div>
            @endif
            <div class="user-meta"><b>{{ $user->name }}</b><br><span>{{ $user->role }}</span></div>
            <form method="POST" action="{{ route('auth.logout') }}">@csrf<button class="btn btn-light" type="submit">Keluar</button></form>
        </div>
    </div>
</header>
