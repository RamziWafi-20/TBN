...
<body>
    <div class="auth-wrapper">
        ...
        <div style="margin-bottom: 1.25rem;">
            <a href="{{ url('/') }}" style="display: inline-flex; align-items: center; gap: 0.4rem; font-size: 0.875rem; font-weight: 600; color: #059669; text-decoration: none;">
        ...
        <div class="auth-card">
            ...
            <!-- LOGIN FORM -->
            <form id="form-login" method="POST" action="{{ route('auth.login') }}">
                @csrf
                <!-- Role Selection -->
                ...
                <div style="margin-bottom: 1.15rem;">
                    ...
                    <input type="text" id="login-email" name="email" class="form-input" placeholder="contoh: siswa@neskar.sch.id" value="{{ old('email') }}" required autofocus>
                </div>
            ...
            <!-- REGISTER FORM -->
            <form id="form-register" method="POST" action="{{ route('auth.register') }}" style="display: none;">
                @csrf
                <div style="margin-bottom: 1rem;">
                    <label for="reg-name" class="form-label">Nama Lengkap</label>
                    <input type="text" id="reg-name" name="name" class="form-input" placeholder="contoh: Muhammad Rizky" value="{{ old('name') }}" required>
                </div>

                <div style="margin-bottom: 1rem;">
                    <label for="reg-email" class="form-label">Email Sekolah</label>
                    <input type="email" id="reg-email" name="email" class="form-input" placeholder="nama@neskar.sch.id" value="{{ old('email') }}" required>
                </div>

                <div style="margin-bottom: 1rem;">
                    <label for="reg-nis" class="form-label">NIS / NISN (Opsional)</label>
                    <input type="text" id="reg-nis" name="nis" class="form-input" placeholder="contoh: 212210045" value="{{ old('nis') }}">
                </div>

                <div style="margin-bottom: 1rem;">
                    <label for="reg-class" class="form-label">Kelas <span style="color:#94a3b8;font-weight:500">(Opsional)</span></label>
                    <input type="text" id="reg-class" name="class_name" class="form-input" placeholder="contoh: XII RPL 2" value="{{ old('class_name') }}">
                </div>
        ...
        <p style="text-align: center; font-size: 0.75rem; color: #94a3b8; margin-top: 1.5rem;">
            © {{ date('Y') }} TBN — Trash Bank Neskar. Bank Sampah Digital SMKN 1 Karawang.
        </p>
    </div>
